...
object ClientLogonChallenge {
  ...
  val reversedCString = reverse(cstring)
  final val GameName = reversedCString encode "WoW" require
  ...
  implicit val codec: Codec[ClientLogonChallenge] = {
    ("opCode" | uint8L) ::
      ...
      ("versionPatch" | uint8L) ::
      ("build" | uint16L) ::
      ("platform" | fixedSizeBytes(PlatformLength, reversedCString)) ::
      ("os" | fixedSizeBytes(OSLength, reversedCString)) ::
      ("country" | fixedSizeBytes(CountryLength, reversedAscii)) ::
