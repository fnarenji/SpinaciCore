...
object Srp6Constants {
  ...
  // Hashes for g and N
  val NBytesDigest = messageDigest.digest(NBytes)
  val gBytesDigest = messageDigest.digest(gBytes)

  // XOR both digest to get t3
  val gDigestXorNDigest = NBytesDigest.zip(gBytesDigest).map { case (a, b) => a ^ b }.map(_.toByte)
